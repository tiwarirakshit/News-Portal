...
import {
  multerFileTypeFilterForImage,
  MULTER_UPLOAD_SIZE_LIMIT,
...
const upload = multer({
  ...
  fileFilter: multerFileTypeFilterForImage,
});
...
router.get(
  '/getNewsUpdatePage/:id',
  authenticateAdmin,
  newsController.getUpdatePage,
);

router.post(
  '/updateNews/:id',
  authenticateAdmin,
  requestValidator.updateNews,
  upload.array('image', 5),
